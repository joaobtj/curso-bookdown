...
github-repo: joaobtj/curso-bookdown
...
* `date`: data de publicação. Para buscar automaticamente data em que foi feita a última modificação, use a função `Sys.Date()`.
...
* github-repo: repositório do github no formato `"usuario/repositorio"`
* `site`: para renderizar o e-book no RStudio, definir a opção como `bookdown::bookdown_site`.
...
* `link-citation`: definir como `yes` para as citações também serem links.
