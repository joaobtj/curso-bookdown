...
Os arquivos de configuração são escritos na linguagem YAML (https://pt.wikipedia.org/wiki/YAML). São eles: `_bookdown.yml` ([Seção o arquivo `_bookdown.yml`](#bkd)), `_output.yml` ([Seção o arquivo `_output.yml`](#output)). Além disso, mais configurações são adicionadas ao cabeçalho do primeiro arquivo Rmd (`index.Rmd` \@ref(index)) ([Seção O arquivo `index.Rmd`](#index)).
